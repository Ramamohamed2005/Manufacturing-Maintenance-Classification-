...
## Modeling: Predictive Maintenance Classification

The file `maintenance_modeling.py` implements the modeling phase for predicting equipment failures and maintenance needs from the unified warehouse table `maintenance.FactSensorReadings`.

### Algorithms Implemented

1. **Deep Neural Network** - multilayer feed-forward model for complex sensor patterns.
2. **Random Forest Classifier** - tree ensemble model for nonlinear sensor patterns.
3. **Gradient Boosting Classifier** - boosted model for stronger predictive performance on difficult failure boundaries.

### Output Classes

The models estimate failure probability, then convert it into three operational statuses:

| Status | Rule | Maintenance Strategy |
| :--- | :--- | :--- |
| Normal | Failure probability below 35% | Continue routine monitoring |
| Maintenance Required | Failure probability from 35% to below 70% | Schedule preventive maintenance |
| Failure Imminent | Failure probability 70% or higher | Inspect within 24 hours and prepare spare parts |
...
Train from the PostgreSQL data warehouse:

```bash
python maintenance_modeling.py
```

Run with generated demo data if the database is not available:

```bash
python maintenance_modeling.py --demo
```

Optional row limit for faster testing:

```bash
python maintenance_modeling.py --limit 50000
```

### Generated Files

The script writes results into `model_outputs/`:

- `model_metrics.csv` - accuracy, failure F1-score, confusion matrix, and reports for all three algorithms.
- `maintenance_recommendations.csv` - equipment-level predicted status and proactive maintenance action.
- `best_failure_classifier.joblib` - saved best-performing model.
- `modeling_summary.md` - concise summary for reporting.
